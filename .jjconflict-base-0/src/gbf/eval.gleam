...
pub fn eval(
  node: AST,
  vm_result: Result(VirtualMachine, String),
...
fn eval_command(
  ...
  case command {
    #(token.StartBlock, _) -> Error("unexpected command: start block")
    #(token.EndBlock, _) -> Error("unexpected command: end block block")
    ...
    #(_, pos) -> Error("unexpected command at " <> pos.offset |> int.to_string)
  }
}
...
fn increment_pointer(vm: VirtualMachine) {
  ...
  case pointer >= vm.tape_size {
    True -> Error("Pointer ran off tape")
...
fn increment_byte(vm: VirtualMachine) {
  ...
  case new_cell_value > vm.cell_size {
    ...
    False -> vm.set_cell(vm.pointer, new_cell_value, vm) |> Ok
  }
}
...
fn input_byte(vm: VirtualMachine) {
  case vm.input {
    ...
    [first, ..] -> {
      ...
      let vm = vm.set_cell(vm.pointer, first, vm)
...
fn output_byte(vm: VirtualMachine) {
  ...
  case to_char(cell_value) {
    None -> Error("invalid ansi char " <> int.to_string(cell_value))
    Some(char) -> {
